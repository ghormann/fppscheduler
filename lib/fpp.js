/**
 * fpp — HTTP client for the Falcon Player (FPP) REST API.
 *
 * FPPApi wraps the FPP JSON endpoints used by the scheduler: status polling,
 * playlist control, and the LED overlay model used for scrolling text displays.
 *
 * All methods are non-throwing — on network or HTTP errors, axiosWrapper logs
 * the failure and returns the provided fallback object so callers never need
 * defensive try/catch around individual API calls.
 */
const axios = require("axios");

class FPPApi {
    /** @param {string} ip_addr  IP address or hostname of the FPP instance */
    constructor(ip_addr) {
        this.ip_addr = ip_addr;
        this.model_name = "LED Panels";
    }

    /** Returns the current FPP playback status, including active playlist and seconds remaining. */
    async getFppStatus() {
    ...
    /** Starts the named playlist immediately on this FPP instance. */
    async playNow(playlist, reason="") {
        console.log("Starting :", playlist, " Reason:", reason);
...
/**
 * Sends an HTTP request and returns parsed response data.
 * On any error (network, timeout, non-2xx), logs diagnostics and returns
 * `fallback` so all callers get a consistent object shape regardless of failure.
 *
 * @param {string} url
 * @param {object} fallback  Value returned on any error
 * @param {string} [method]  HTTP method (default GET)
 * @param {string|null} [data]  Request body (JSON string)
...
async function axiosWrapper(url, fallback, method='GET', data = null, timeout=2000) {
    try {
        ...
        console.log("In Axios Error Handler for URL:", url);
