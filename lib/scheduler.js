...
function findNextInternal() {
    ...
    if (answer != "") {
        dataModel.internal_songs.forEach(function(s) {
            ...
            console.log('DEBUG: ', s.playlist, ": ", s.next.toLocaleDateString('en-US', {timeStyle: 'long'}));
        });
    }
...
async function doScheduleCheck() {
    ...
    //console.log("isDisplayHours: ", dataModel.current.isDisplayHours, ", Clock: ", dataModel.clock.time);
    if (dataModel.current.isDisplayHours) {
        dataModel.playGoodnight = true;
    }
    let fppStatus = await getFppStatus();
    fppStatus = fppStatus.data;
...
async function playNow(playlist) {
    ...
    let url = "http://" + process.env.FPP_HOST + "/api/playlist/" + playlist + "/start";
...
function getFppStatus() {
    return axios.get("http://" + process.env.FPP_HOST + "/api/fppd/status");
}
