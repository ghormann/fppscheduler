/**
 * scheduler — main show scheduler for the FPP Christmas display.
 *
 * Two entry points are called on a timer from index.js:
 *   doScheduleCheck() — runs every ~0.5 s.  Polls FPP for current playback
 *     status and decides what to start next based on: time of day, admin
 *     overrides, the name-display queue, internal interstitial playlists,
 *     and the top-voted song.
 *   doNameCheck()     — runs every few seconds.  Triggers name-sequence
 *     generation and broadcasts an estimated wait time to the MQTT bus.
 *
 * isDisplayHours() controls whether the show is active.  It returns true
 * during the nightly display windows and on special calendar dates:
 * Christmas Eve after 5 pm, Christmas morning before 1 am, New Year's Eve
 * after 5 pm, New Year's Day before 1 am, and whenever debug mode is on.
 */
"use strict";
...
/**
 * Calculates a human-readable wait estimate and publishes it via MQTT.
 * If `msg` is null a message is auto-generated from `seconds`.
 * Negative seconds are clamped to 0; estimates over 10 minutes use a
 * fixed "about 10 minutes" message to avoid alarming visitors.
 * When Internal_Wish_Name is playing the estimate is always "now".
 *
 * @param {number}      seconds  Estimated seconds until the name plays
 * @param {string|null} msg      Override message, or null to auto-generate
 */
...
/**
 * Called every few seconds to manage name-sequence generation.
 * Handles three distinct situations:
 *   1. Outside display hours — sends a "show not started" estimate.
 *   2. Near midnight (clock < 150 s) — forces a GENERATE_MIDNIGHT action.
 *   3. Normal hours (IDLE queue) — triggers GENERATE when the queue has been
 *      waiting too long relative to current queue depth.
 */
function doNameCheck() {
    ...
    if (dataModel.current.nameStatus == "READY") {
        setNameEstimate(dataModel.seconds_remaining);
    }

    if (dataModel.current.nameStatus == "GENERATING") {
        let gen_dur_secs = (moment().toDate() - dataModel.current.lastNameGen) / 1000;
        if ((35 - gen_dur_secs) < dataModel.seconds_remaining) {
            setNameEstimate(dataModel.seconds_remaining);
        }
    }
    ...
    let diff_gen = Date.now() - dataModel.current.lastNamePlay;
    ...
    if (dataModel.nameQueue.status === "IDLE") {
        ...
        // 240000 here (4 minutes) is because most songs take 2 minutes. 12000 (2 min) is really used to
        // control name gen
        let name_estimate = Math.round(Math.max(max_dur - diff, 240000 - diff_gen, dataModel.seconds_remaining * 1000) / 1000);
...
/**
 * Scans internal_songs for the first enabled entry whose scheduled time has
 * passed, plays it, and advances its next-play time by frequency_min.
 * Also pushes all other enabled songs forward by 2 minutes to avoid
 * back-to-back internal interruptions.  Returns the playlist name or "".
 */
...
/**
 * Main scheduling loop — called every ~0.5 s.
 * Throttles to at most one FPP command per 3 seconds, then:
 *   • Plays Internal_Midnight when the countdown clock reaches zero.
 *   • When FPP is idle: admin override → name-display → internal interstitial
 *     → top-voted song (priority order).
 *   • When FPP is playing: tracks seconds_remaining, resets name status after
 *     the name sequence finishes, and marks midnight as started.
 */
async function doScheduleCheck() {
    ...
    let sequence = "";
    if ("current_sequence" in fppStatus) {
        sequence = fppStatus.current_sequence;
    }
    dataModel.current.sequence = sequence;
    ...
    if (currentSong.count == "0" || currentSong.playlist === "Internal_off") {
        ...
        if (dataModel.current.sequence === "Wish_Name.fseq") {
            dataModel.current.lastNamePlay = moment().toDate();
            if ("READY" === dataModel.current.nameStatus) {
                if (dataModel.seconds_remaining < 8 && dataModel.seconds_remaining > 2) {
                    // Waiting until last 10 seconds to hold names longer in the queue for website
                    console.log("Resetting name status after playing name");
...
/** Starts a playlist on the main FPP display and notifies smart plugs. */
...
/**
 * Plays the current top-voted song, or a random song from the full playlist
 * if no vote is pending.  Clears the topVote after consuming it.
 */
...
/**
 * Returns true when the light show should be running.
 * Active windows:
 *   • Nightly: 6:00–8:59 am and 5:00–10:59 pm every day in December
 *   • Christmas Eve (Dec 24) after 5 pm
 *   • Christmas morning (Dec 25) before 1 am
 *   • New Year's Eve (Dec 31) after 5 pm
 *   • New Year's Day (Jan 1) before 1 am
 *   • Always true when dataModel.current.debug is set
 */
...
module.exports.findNextInternal = findNextInternal;
module.exports.setNameEstimate = setNameEstimate;
